            ...
            <div class="ibox-content p-xl" id="Print">
                <div class="row">
                    ...
                    <div class="col-sm-4 text-center">
                        <address>
                            <h1><strong>Modern life care</strong></h1>
                ...
                <table class="table table-bordered">
                    <tr>
                        <td>Invoice No : {{ $tracking_code }}</td>
                   ...
                   {{--  @php
                    ...
                    @endphp --}}
                        <tr>
                            <td>{{ $content->name }}</td>
                            <td>{{ $content->qty }}</td>
                            <td>{{ $content->options->pv }}</td>
                            <td>৳{{ $content->price }}</td>
                            <td class="text-right">{{ $content->options->pv * $content->qty }}</td>
                            <td>৳{{ $content->price * $content->qty }}</td>
                        </tr>
                        <?php $sum_tot_Price += $content->qty * $content->options->pv ?>
                        <?php $sum_unit_Price += $content->qty ?>
                    @endforeach
                        </tbody>
                    </table>
                </div>
                ...
                <table class="table invoice-total">
                    <tbody>
                        ...
                        <tr>
                            <td id="print-none"><strong>discount :</strong></td>
                            <td id="print-none">
                                <form action="{{ url('discount') }}" method="POST">
                                    @csrf
                                    <select name="id" class="form-control" hidden>
                                        <option value="{{ $tracking_code }}">{{ $tracking_code }}</option>
                                    </select>
                                    <input type="text" class="form-control mb-2" name="discount">
